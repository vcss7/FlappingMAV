...
!REAL(KIND = R8), DIMENSION(1) :: Fresults ! FMIN results for verification.
...
!REAL(KIND = R8), DIMENSION(1,6) :: Xresults ! X results for verification.
...
IF (proc_id == 0) THEN
  ...
  IF(ALL(status < 10)) THEN ! Normal return.
    WRITE (*,113) objnm,n_masters,FMIN,diam_lim,&
                  iter_lim,eval_lim
